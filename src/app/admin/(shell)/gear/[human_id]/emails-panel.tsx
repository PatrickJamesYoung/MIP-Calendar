...
import type { EmailMessage } from "./page";

/**
 * Right-column subsection that lists every email tied to this
 * reservation — sent (outbound) and, once PR C ships, replies
 * (inbound). Each row expands to reveal the full plaintext body.
 *
 * We deliberately render the plaintext copy (body_text) rather than
 * the stored HTML. Two reasons:
 *   1. Reservations' outbound emails come from operator-editable
 *      templates. Rendering arbitrary HTML in the admin UI is a
 *      needless XSS surface, however contained.
 *   2. Admins want to *read* the message, not preview how it looked
 *      in Gmail. Plaintext with preserved line breaks is faster to
 *      scan.
 */
