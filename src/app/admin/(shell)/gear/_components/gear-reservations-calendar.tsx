...
function buildMonthGrid(cursor: Date, reservations: CalendarReservation[]): Grid {
  ...
  for (const r of reservations) {
    ...
    // Clip to grid.
    // daysBetween(a, b) is b - a, so `< 0` on (gridStart, pickup) means
    // pickup < gridStart (clamp to gridStart); `> 0` on (ret, gridEnd)
    // means gridEnd > ret (reservation ends within grid, use ret).
    ...
    while (daysBetween(cursorDay, visEnd) >= 0) {
      ...
      // Clip this week's segment to the earlier of (visible end, week end).
