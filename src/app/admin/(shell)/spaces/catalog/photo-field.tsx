...
import { uploadSpaceImageAction } from "./actions";

/**
 * Combined "Photo URL" + "Upload image" field for the spaces catalog admin.
 * Mirror of the gear catalog PhotoField, wired to the spaces uploader.
 */
export function PhotoField({
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const result = await uploadSpaceImageAction(null, formData);
      if (result.ok) {
        setUrl(result.url);
        setStatus({ kind: "ok" });
      } else {
      ...
      <div className="flex flex-col gap-2">
        ...
        {url && status.kind !== "ok" && (
          ...
          <img
            src={url}
            alt=""
            className="mt-1 h-20 w-auto rounded border border-neutral-200 object-cover"
            onError={(e) => {
